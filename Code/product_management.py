...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.addProduct_button_2.setText(_translate("MainWindow", "Add Product"))
        self.voidProduct_button_2.setText(_translate("MainWindow", "Void Product"))
    ...
    def highlight_qty_cells(self):
        for row in range(self.tableWidget.rowCount()):
            qty_item = self.tableWidget.item(row, 5)  # Assuming the "Qty" column is at index 5
            if qty_item is not None:
                qty = int(qty_item.text())
                if qty <= 5:
                    self.set_cell_background_color(row, 5, "#ffcccc")  # Highlight red
                elif 5 < qty < 15:
                    self.set_cell_background_color(row, 5, "#ffcc99")  # Highlight orange
                else:
                    self.set_cell_background_color(row, 5, "#ccffcc")  # Highlight green
    ...
    def load_data(self):
        ...
        cur.execute("SELECT product_name, brand, var, size, price, qty FROM products")
        products = cur.fetchall()

        # Display data in the tableWidget
        self.tableWidget.setRowCount(len(products))
        self.tableWidget.setColumnCount(6)  # Adjusted column count to match the table structure
        self.tableWidget.setHorizontalHeaderLabels(["Product Name", "Brand", "Var", "Size", "Price", "Qty"])
        ...
        self.highlight_qty_cells()  # Add this line to call the highlighting function
